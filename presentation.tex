...
\item
  Anyone can change one number or one step and rerun, with no hardcoded
  values to hunt down.
\end{itemize}
\end{frame}
...
\item
  Be as descriptive as you can in folder and file names; avoid special
  characters.
\item
  Prefer tool-agnostic file formats: a ``\emph{.csv}'' opens anywhere,
  for anyone.
\item
...
\highlightbox{Write for the stranger who inherits the project, because in two years that stranger is you.}
\end{frame}
...
\includegraphics[width=2.34375in,height=\textheight,keepaspectratio]{images/paste-7.png}
...
quarto}'', ``\emph{make appendix}'', ``\emph{make paper}'', or
``\emph{make slides}'' \begin{center}
...
``\emph{make quarto}'' renders the self-contained report
...
first the damage, then the shared rulebook that prevents it, then the
two messages that survive any tool change (you stay in the loop, and
AI-touched data is generated data), and at the end advice for your own
personal setup.}
\end{frame}
...
and agentic systems, a convention most agentic tools now read on their
own, including OpenAI Codex, Cursor, and Hermes. This is the answer to
the opening problem: one tool-neutral file serves whichever agent each
coauthor brings. Two widely used tools do not read it yet, Claude Code
and Gemini CLI, and the repository's README gives the one-line fix: a
CLAUDE.md or GEMINI.md that points at AGENTS.md. That pointer file stays
on your own machine, so the shared repository still ships one rulebook.
The bullets on the left are the file's actual rules. It helps in 1\_code
and 4\_drafts only. Raw data stays read-only, with one exception: the
codebook is hand-maintained documentation, and the agent must keep it in
sync with the data. Intermediates and outputs are rebuilt by the
pipeline, never edited by hand. Credentials never enter the repo, the
same environment-variable rule from chapter one. Two rules are newer.
Before a change that touches more than one file, the agent states its
plan first. And it cleans up any scratch files it creates: the folder
structure is fixed. The screenshot shows the rules; below them, the file
hands the agent the make targets and ends with a verification checklist:
rebuild, confirm the drafts regenerated, run git status, and report
anything it could not verify. The line to remember: it should work is
not verification. And the rule that anchors it all bans git commit and
git push: the agent leaves the diff for us. The next three slides are
...
\agentrule{Help the user make changes in 1\_code/ (analysis), 4\_drafts/ (paper, slides, and data appendix sources), and 0\_data/codebook.md only.}
...
also touched code.do, run make stata too. R and Stata both write the
same table file, so comparing the two catches drift, like the two
defining percentiles differently. code.qmd is the self-contained
alternative and writes nothing the drafts read, so make quarto only
confirms that report still renders. One more reason the rerun matters:
agents are built to please, and pushed for a result they will sooner
produce a plausible number than admit failure. Our own make run is the
one test that cannot be flattered.}
\end{frame}
...
\begin{frame}{4. AI agents: The researcher stays in the loop}
\phantomsection\label{ai-agents-the-researcher-stays-in-the-loop}
\begin{itemize}
\tightlist
\item
...
\item
  The pipeline hands you the gates: read the diff, rerun \emph{make},
  check the trail
\item
...
\highlightbox{Stay in the loop: nothing an AI produced enters your paper unverified.}

\note{The last three slides were mechanics: read the diff, rerun the
pipeline, keep the trail. This slide is the principle behind them, and
the first of two messages I want you to take home from this chapter. The
division of labor is fixed: the AI assists, and we remain author,
analyst, and judge. It can draft code, propose a merge, suggest a
robustness check. It does not decide what enters the paper. That
decision passes through verification, and verification means checking
exactly what we would check for a new coauthor: the sources exist, the
numbers rebuild, the code does what it claims, the interpretation is
ours. The pipeline makes this cheap. The diff shows what changed, make
shows whether it still rebuilds, the commit history shows what happened
over time. But a gate only works if someone stands at it. If we wave the
agent's work through, the gates are decoration. The German research
funder DFG puts it in one line: responsibility for what is published
remains with the researcher, never with the tool.}
\end{frame}

\begin{frame}{4. AI agents: When AI touches data, it becomes generated
data}
\phantomsection\label{ai-agents-when-ai-touches-data-it-becomes-generated-data}
\begin{itemize}
\tightlist
\item
  Raw data stays untouched in ``\emph{0\_data}'': the record of what
  actually happened
\item
  Once AI cleans, codes, merges, or edits a data file, the output is
  generated data: the model's choices are inside it
\item
  Structuring data is a research act, even when software performs it
\item
  Keep every AI transformation as code in ``\emph{1\_code}'':
  rerunnable, reviewable, visible in the diff
\item
  Disclose the touch: tool, task, your verification, and whether AI
  output entered the final data
\end{itemize}

\bigbreak

\highlightbox{Data an AI touched is generated data, not naturally occurring data. Keep the transformation in code, and disclose it.}

\note{The second message is about the data itself. Our field
distinguishes data we merely observe from data someone shaped.
Bloomfield, Nelson, and Soltes (Bloomfield, Nelson, and Soltes 2016)
call structuring one of the five data-gathering tasks: turning records
into analyzable datasets involves choices about what to keep, how to
code it, what to drop. And they are explicit that hand-collection is a
metaphor: a computer program that processes documents is structuring
data all the same. So when an AI cleans a file, codes open answers,
fills gaps, or merges two sources, it has performed a research act on
our data. The output is generated data. The model's choices are baked
in, and they are invisible in the file itself: ask the same model
tomorrow and you get a different file. If you know this framework, you
may object that Compustat is also structured by someone else and
archival researchers still call it naturally occurring. But a vendor's
structuring is documented, stable, and shared by the whole field, and it
happened before and independent of our research question. An AI
transformation happens inside our project, steered by our prompt: its
choices are our choices, delegated, and they are written down nowhere
unless we keep them in code. That is why the touch must live in code. If
the transformation sits in 1\_code, it reruns, the diff shows every
choice, and the codebook says where each variable came from. That is
also the disclosure story. When a method section or a journal asks what
AI did to the data, the commit history and the codebook already contain
the answer: the tool, the task, how it was verified, and whether AI
output entered the final dataset. Others keep a separate AI-use log for
this; with the pipeline, the log already exists. Hiding the touch is
what turns a helpful tool into a validity problem.}
\end{frame}
...
\begin{frame}{4. AI agents: Skill design for academic work}
\phantomsection\label{ai-agents-skill-design-for-academic-work}
\centering

\includegraphics[width=0.98\linewidth,height=\textheight,keepaspectratio]{images/skill_design.png}

\note{This is my own skill design, and it is the shape of every skill I
run on research and teaching work. The model plans, and I approve the
plan before anything is executed. It then does the work and reviews
itself, but a producer checking its own work is not independent, so the
result goes to a fresh agent that only audits. Those audit rounds run
like revise and resubmit, at most three, and whatever is still contested
comes to me. Two gates are mine: the plan and the result. Between them I
expect not to be needed, and what the process leaves behind, the plan,
the comments, the responses, and my decisions, is the account of how the
work was made.}
\end{frame}
...
\item
  ...
  \item
    ...
    \item
      If AI touched the data, it is generated data: keep the
      transformation in code and disclose it
    \end{itemize}
  \end{enumerate}
\item
...
\bibitem[\citeproctext]{ref-BNS_2016}
Bloomfield, Robert, Mark W. Nelson, and Eugene Soltes. 2016.
{``Gathering Data for Archival, Field, Survey, and Experimental
Accounting Research.''} \emph{Journal of Accounting Research} 54 (2):
341--95. \url{https://doi.org/10.1111/1475-679X.12104}.
...
\bibitem[\citeproctext]{ref-FGHKO_2024}
...
\bibitem[\citeproctext]{ref-HLL_2020}
