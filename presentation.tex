...
\ProvidesFile{talk_template.sty}[2026/07/30 WHU beamer theme v4.2]
...
% Quarto's lualatex path loads fontspec + unicode-math BEFORE this file.
% In that case, set a real math font so \times, \rightarrow, the math
% minus, and \checkmark render (they silently dropped before v4).
% Without fontspec (hand-written .tex via xelatex), keep the legacy setup.
...
\setbeamertemplate{background}{%
  \begin{tikzpicture}
    ...
    \ifnum\thepage=1\relax
      ...
      \fill[whu_lightblue] (0,0) rectangle(1.5,1.9);
...
{%
  ...
  % the two 1.5cm gaps can shrink: a long multi-line title then compresses
  % them instead of pushing the author block down into the bottom bar, where
  % it would overprint the institute line (blue on blue, ghost strokes)
  \vskip1.5cm minus 1.1cm
...
{%
  ...
  \begin{beamercolorbox}[wd=\paperwidth,ht=0.5cm]{frametitle}
    \begin{tikzpicture}
      ...
      \fill[whu_lightblue] (0,0) rectangle(1.5,1.2);
...
\newcommand{\sectionslide}[1]{%
  \begin{frame}[plain]
    ...
    \begin{tikzpicture}
      ...
      \fill[whu_lightblue] (0,1.0) rectangle(1.5,2.2);
...
% Deprecated box aliases - kept so pre-v4 decks compile; all of them
% now render as a \highlightbox with their printed label preserved.
\newcommand{\discussionprompt}[1]{\highlightbox{#1}}
\newcommand{\how}[1]{%
  \highlightbox{\textbf{\textcolor{whu_blue}{How?}} #1}}
\newcommand{\takeaway}[1]{%
  \highlightbox{\textbf{\textcolor{whu_blue}{Key takeaway:}} #1}}
