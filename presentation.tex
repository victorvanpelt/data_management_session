...
Bloomfield, Nelson, and Soltes call structuring one of the five
data-gathering tasks: turning records into analyzable datasets involves
choices about what to keep, how to code it, what to drop. And they are
explicit that hand-collection is a metaphor: a computer program that
processes documents is structuring data all the same. So when an AI
cleans a file, codes open answers, fills gaps, or merges two sources, it
has performed a research act on our data. The output is generated data.
The model's choices are baked in, and they are invisible in the file
itself: ask the same model tomorrow and you get a different file. If you
know this framework, you may object that Compustat is also structured by
someone else and archival researchers still call it naturally occurring.
But a vendor's structuring is documented, stable, and shared by the
whole field, and it happened before and independent of our research
question. An AI transformation happens inside our project, steered by
our prompt: its choices are our choices, delegated, and they are written
down nowhere unless we keep them in code. That is why the touch must
live in code. If the transformation sits in 1\_code, it reruns, the diff
shows every choice, and the codebook says where each variable came from.
That is also the disclosure story. When a method section or a journal
asks what AI did to the data, the commit history and the codebook
already contain the answer: the tool, the task, how it was verified, and
whether AI output entered the final dataset. Others keep a separate
AI-use log for this; with the pipeline, the log already exists. Hiding
the touch is what turns a helpful tool into a validity problem.}
\end{frame}
